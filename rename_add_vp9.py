#!/usr/bin/env python3
"""
批量重命名视频文件 - 添加 _vp9 后缀

功能：
- 递归扫描当前目录及子目录
- 查找视频文件
- 重命名文件，在扩展名前添加 _vp9 后缀
- 支持预览模式（dry-run）
...
def rename_add_vp9(root_dir=".", dry_run=True, file_types=None):
    """
    重命名视频文件，添加 _vp9 后缀
    ...
    # 查找所有视频文件（排除已有 _vp9 后缀的）
    files_to_rename = []
    for file_type in file_types:
        for file_path in root_path.rglob(f"*{file_type}"):
            if file_path.is_file() and not file_path.stem.endswith("_vp9"):
                files_to_rename.append(file_path)

    if not files_to_rename:
        print("未找到需要添加 _vp9 后缀的视频文件")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='批量重命名视频文件，添加 _vp9 后缀',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 预览模式：查看将要重命名的文件（默认，不执行）
  python3 rename_add_vp9.py

  # 预览指定目录
  python3 rename_add_vp9.py --dir /path/to/videos

  # 执行实际重命名
  python3 rename_add_vp9.py --execute

  # 在指定目录执行重命名
  python3 rename_add_vp9.py --dir /path/to/videos --execute

  # 只处理特定格式（例如只处理 webm 和 mp4）
  python3 rename_add_vp9.py --execute --types .webm .mp4
